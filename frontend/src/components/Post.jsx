...
const Post = ({ post, onLike }) => {
  const initialLiked = post.likedByCurrentUser ?? false;
  ...
  return (
    <div className="bg-gray-800 rounded-lg p-6 mb-6 shadow-lg border border-gray-700 max-w-xl mx-auto">
      <div className="flex items-center mb-3">
        <img src={post.author.picture || "/default-avatar.png"} alt="author" className="w-12 h-12 rounded-full mr-3" />
        <div>
          <div className="font-bold text-green-400 text-lg">{post.author.name}</div>
        </div>
      </div>
      <hr className="mb-4 border-gray-600" />
      <h2 className="text-xl font-bold text-white mb-2">{post.title}</h2>
      {post.picture && <img src={post.picture} alt="Post" className="w-full max-h-72 object-cover rounded-lg mb-3" />}
      ...
      <div className="flex items-center space-x-2 text-gray-400 text-lg mt-2">
        <button
          ...
          title={isLiked ? "Anulează like-ul" : "Like"}
        >
          <Heart
